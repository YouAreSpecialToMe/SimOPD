...
#   slot1  h9_prune_adapt  GPUs 2,3   (自适应预算)
...
if [ -z "${MLP_ROLE_INDEX:-}${MLP_WORKER_RACK_RANK_INDEX:-}${DLC_JOB_ID:-}" ]; then
    cat <<CARD
========= DLC 控制台表单(H 轴,每 seed 一作业,1 worker × 8 GPU) =========
  任务名称   simopd-h-axis-5lane-s${SEED}
  节点数量   1
  ...
  前置:$D/a_axis/ 下须有五个 rehearsal_h*.OK(彩排在 gpu193 做,不在 DLC 重跑)
==========================================================================
...
BAD=$(grep 'PROBLEM' "$LINT_LOG" \
      | grep -E 'h6_gen_sched|h7_gen512|h8_gen2048|h9_prune_adapt|h10_task_subset' \
...
# --------------------------------------------------------- Phase P' 资产断言 --
for f in "$D/gkd_offpolicy.parquet.dry" "$D/simopd_math/train_sub50.parquet"; do
    if [ ! -f "$f" ]; then
        while true; do _abort_check; echo "ASSET MISSING: $f -- lanes NOT launched ($(date))"; sleep 600; done
    fi
done
echo "== Phase P': assets present (.dry keys + train_sub50)"
...
_launch_lane() {  # ARM PAIR  -- 有界重试,run_opd_baseline 从 ckpt 自恢复
    ...
    for attempt in 1 2 3; do
        (
            ...
            export WANDB_TAGS="${ARM},H,seed${SEED},dlc_h_axis"
...
( _launch_lane h9_prune_adapt  2,3 ) > "$LOGD/lane_h9_prune_adapt_s${SEED}.log"  2>&1 & _lpids+=($!)
...
echo "lanes launched: h6->0,1 h9->2,3 h10->4,5 h7(->h8 chain)->6,7 (logs $LOGD/lane_h*_s${SEED}.log)"
