...
unset STEPS TEST_FREQ SAVE_FREQ TAG REHEARSAL PROJECT_NAME EXPERIMENT_NAME \
      ...
      TOTAL_TRAINING_STEPS MAX_RESPONSE_LENGTH DISTILLATION_LOSS_MODE \
      SIMOPD_USE_MODELSCOPE VERL_USE_MODELSCOPE VLLM_USE_MODELSCOPE \
      ROLLOUT_N TRAIN_BATCH_SIZE PPO_MINI_BATCH_SIZE USE_TASK_REWARDS \
      DISTILLATION_LOSS_COEF DISTILLATION_TOPK USE_POLICY_GRADIENT \
      LOSS_MAX_CLAMP LOG_PROB_MIN_CLAMP STUDENT_MODEL TEACHER_MODEL ACTOR_LR \
      MAX_PROMPT_LENGTH PPO_MAX_TOKEN_LEN_PER_GPU TOTAL_EPOCHS VAL_BEFORE_TRAIN \
      USE_REMOVE_PADDING MAX_CKPT_KEEP DATA_DIR TRAIN_FILE_BASENAME LOGGER \
      MANIFEST 2>/dev/null
# Arm knobs re-enter per run through arm.py env -- the one sanctioned channel. A
# leaked operator export would otherwise reshape EVERY lane (the STEPS=3 class;
# audit 2026-08-07 F4). ModelScope stays force-unset above.
for _v in $(compgen -v | grep '^SIMOPD_' 2>/dev/null); do unset "$_v" 2>/dev/null; done
