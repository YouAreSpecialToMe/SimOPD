...
# Re-running setup OVERWRITES this file. Anything added to it by hand -- the
# fleet's WANDB_API_KEY lives there, deliberately outside git -- must be re-added
# after a regeneration; the failure mode is only degraded (runs fall back to
# offline wandb, visibly), never wrong.
cat > "$ENV_FILE" <<ENVEOF
...
export HF_ENDPOINT="${HF_ENDPOINT:-https://huggingface.co}"
export HF_HUB_DISABLE_XET="${HF_HUB_DISABLE_XET:-1}"
# Resolve every model from the local cache, no hub API calls. The whole fleet
# shares ONE egress IP and the unauthenticated budget is 500 requests/300s FOR THE
# FLEET: sixteen vLLM engines starting together each list their repo tree, the
# window empties in seconds, and every teacher init dies with
#   429 Too Many Requests ... We had to rate limit your IP
# -- which killed all sixteen wave-1 launches at once (2026-08-07). Assets are
# pre-fetched above and verified by fetch_assets --check, so these calls were pure
# liability. Set HF_HUB_OFFLINE=0 only to fetch something new (--repair).
