...
.pan .ph{display:flex;align-items:baseline;gap:7px}
.pan .pt{font-size:12.5px;font-weight:620}
...
function domain(o,arms){
  ...
  if(hi===lo)hi=lo+1e-6;
  if(!log){const p=(hi-lo)*.07;lo-=p;hi+=p;if(lo>0&&lo<(hi-lo)*.6)lo=0}
  return {lo,hi,log};
}
...
function drawBase(){
  ...
  panels.forEach(o=>{
    ...
    o.vis.forEach(a=>path(o.base,ptsOf(o,a,S),faint,1,.5));
  });
  drawHi();
}
function drawHi(){
  panels.forEach(o=>{
    ...
    show.forEach(([a,c,w,op])=>{
      ...
      if(last)text(o.hi,Math.min(last[0]+5,S.G.W-4),last[1]+3.5,a.split("_")[0],c,10,"start",600);
    });
    if(cross!=null){
      ...
      o.el.querySelector(".pv").textContent=vals.length
        ? `step ${cross} · `+vals.map(([a,c,v])=>fmt(v)).join(" / ")
        : `step ${cross}`;
      ...
      o.el.querySelector(".pv").textContent=show.length
        ? show.map(([a])=>{const s=MEAN[o.mi][a];if(!s)return"–";
            if(o.m.sparse)return fmt(s[s.length-1][1]);
            for(let i=s.length-1;i>=0;i--)if(s[i]!=null)return fmt(s[i]);return"–"}).join(" / ")
        : `${o.vis.length} 臂`;
    }
  });
}
...
function renderOwn(){
  ...
  D.specific[arm].forEach(p=>{
    ...
    const pad=(hi-lo)*.07;lo-=pad;hi+=pad;if(lo>0&&lo<(hi-lo)*.6)lo=0;
