...
fingerprint=$(printf '%s\n' \
    ...
    "arm=${ARM_ARGS[*]-}" "extra=$*" \
    "simopd=$(env | LC_ALL=C grep '^SIMOPD_' | grep -vE '^SIMOPD_(SHADOW|PI_TAIL_WIDTHS|LANE_TAG|ENTROPY_CHUNK|ARCHIVE|SHORT_RUN_OK|TRAJ_[A-Z_]+|DIV_[A-Z_]+)=' | LC_ALL=C sort | tr '\n' ' ')" \
