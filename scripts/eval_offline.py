...
def main():
    ...
    p.add_argument("--out-dir", default=os.environ.get("SIMOPD_EVALS",
                   os.path.expanduser("~/data/simopd_evals")),
                   help="default matches verdict.py's read dir (audit S1: /scratch is "
                        "node-local on cornell and strands artifacts)")
    args = p.parse_args()
