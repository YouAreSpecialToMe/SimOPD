...
| A1' | **a2 terminator fix (2026-08-19 audit, prerequisite for A1)**: the SFT target ends `<|im_end|>\n` (student-template render), never `<|endoftext|>`; the rollout honors only `<|endoftext|>`; Rethinking's LlamaFactory `qwen3` template has `replace_eos=True` so THEIR SFT'd student stops on `<|im_end|>` — our port lost that. Measured: SFT model p(`<|im_end|>`) 1e-3, p(`<|endoftext|>`) 1e-3 at the end of a teacher-CoT answer (base 0.98), mass on the closing-template continuation; 65% capped at step 1. Fix = terminate the SFT target as `…<|im_end|><|endoftext|>` (teacher's own next token after `<|im_end|>` is `<|endoftext|>` at 1.0), rebuild stage 2 (98 steps, 2 GPUs), then A1's 3 OPD seeds | a2 as run tests an init that cannot terminate by construction, not the recipe; the A axis cannot enter the paper on it | SFT rebuild ~1 GPU·h + A1's ~400 GPU·h; ~10 lines in `src/simopd/sft_dataset.py` |
