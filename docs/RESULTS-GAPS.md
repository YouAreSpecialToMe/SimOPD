...
| N2 | **M-I keystone, supply side — REGISTERED 2026-08-19 as `n2_eos_aux` (wave 17, stock)**: `vanilla + an exact full-softmax teacher EOS-vs-rest calibration term` at every visited state, `BCEWithLogits(m_t, q_t)`, dL/dm = p−q ∈ [−1,1]; nothing else moves (sampled k1, PG, no renorm, token-mean). Payload contract: the stop id is force-gathered from the teacher's full log_softmax (`simopd.eos_gather`, `teacher_patch` N2 reader) — a miss is refused, never read as q=0. Loss set 151643 (`<|endoftext|>`, the token that ends a student rollout); 151645 (`<|im_end|>`) gathered as a diagnostic column | separates what exact top-k does in one move: (a) stop gradient without sampling EOS vs (b) out-of-support deviations never slammed. P-EOS / P-support / P-teacher preregistered in arms.yaml; secondary: splits c2/c4's gain into termination vs content. λ=1 canonical, ladder {0.3,1,3,10} sized by a forward audit A_EOS/A_cont; not stacked with g6 until each moves the fixed point (then the 2×2 V/S/E/S+E) | ~350 GPU·h (2-GPU lane, streaming kernel); CPU battery 49/49; 3-step rehearsal `deploy/dsw/rehearse_n2.sh` pending a GPU node |
...
|---|---|---|
| M1 | **Compression dose-response formalization**: measured effective \|signal\| vs lock step across {vanilla, f2, f1, b1, f3}. **FIRST HARVEST DONE 2026-08-11** (`scripts/m1_signal_tail.py` on the shipped metrics CSV): u_max = {81.6, 10.0, 4.52, 2.303, 0.999} vs locks {122, 198, 208, 247, never} — **strictly monotone, 5/5**; raw spikes near-identical (81/89/91) so the admitted dose, not the exposure, orders; bulk stats (p95/absmean/grad_norm) all FAIL to order — the dose lives beyond ~p99.7 (clip_hit_rate 0.3%). Full entry in the ledger | measurement half **adjudicated**; remaining: add p99/p99.9 to `_QUANTILES` before waves 9–13 (metrics-only); the intervention half is `f2_clip2.3`, its P-M prediction now calibrated (u_max pinned at b1's measured 2.303 ⇒ lock ≈ 247); **top-k family extension** (e2, b2 heavy-tail prediction) still pending |
