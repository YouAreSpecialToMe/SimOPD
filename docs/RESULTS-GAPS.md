...
| N0 | **M-I, proposed 2026-08-19 (NOT registered): the reward-level fix of the stop-token mismatch** — vanilla with one change: at the sampled stop token, Δℓ is the event-level log Σ_{E_T} q − log Σ_{E_S} p instead of log q(`<|endoftext|>`) − log p(`<|endoftext|>`); every other token untouched; same payload as N2 | removes the −25 without adding supply, so it isolates the artifact: stable ⇒ vanilla's collapse is (mostly) the identity mismatch and exact top-k's stability is the exemption; locks ⇒ the continuation drive is real beyond the terminal token | ~350 GPU·h; a dozen-line registry function on N2's payload; user decision |
