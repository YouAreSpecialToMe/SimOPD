...
verifier` 消融格;validity 配方格待按钉死超参重建阶段 1 后补跑。)

(**二次合流更正 2026-08-12**:上句"跨设备 bug 章节未随本次合流带入"已作废 ——
该章节于本次 `main → ch-dev` 合流带入(见文末三节 r6 记录)。两线当时各自修了
`_stat_mask` 的同一处:舰队线让掩码带 device 出生,cornell 线保留 CPU 构造循环
末尾一次性 `.to()`。合流取 **cornell 形式**(CPU 构造更省 kernel launch,且函数
末尾的 `.to(teacher_topk_log_probs.device)` 两线本就共有,设备保证不变),数值恒等。
真正随本次合流首次进入舰队线的是另外两项:**空种群回退 `_pop`**(批级归一化器
遇空微批不再抛)与**影子面板 stat 掩码补传**(c1/c2/e1/e2/e3 五处)—— 后者意味着
**已完成的 16k campaign 里这五个臂的影子面板是在错的种群上算的**,跨臂 Jaccard
(N4/M5)必须在本快照之后重采,旧读数作废。`_student_entropy` 的分块默认取两线
更小的 256 并保留 cornell 的 `SIMOPD_ENTROPY_CHUNK` 覆盖钩子。)

**合流暴露的第三处空种群(c2 的两个 τ 分位数,2026-08-12 修)**:cornell 线的
`_pop` 加固覆盖的是它当时看得见的三个批级归一化器(`_minmax` / `_robust_norm` /
`_topk_by_score`);而 c2 的 τ 分位数是 wave 10 粒度阶梯落地时(08-11,舰队线)
才加的**新分位数消费者**,故从未继承该加固。合流电池(`scripts/merge_equivalence.py`,
本次新增)在"全 prompt 微批"这一格上抓到:`compute_quantile_budget_topk` 的 batch
分支(**即产出 0.684 那条已发表路径**)与 `_qb_tau_per_sequence` 的回退分支都是裸
`torch.quantile(margin[stat])`,空选择直接抛。两处均改走 `_pop`;**非空种群下与旧
表达式逐位相等**,故 c2 已入库的三行跨 pin 仍可比。
意义不止于修一个 bug:它说明两线各自的加固**不会自动组合** —— 一线加固了当时存在
的消费者,另一线随后新增了同类消费者,合流点是唯一能发现这件事的地方。而
`c2_qb_fixed8` / `c2_qb_perseq`(wave 10,6 行)正是这个内核,`perseq` 更是**必走**
那条回退分支 —— 不修就是 6 行踩雷。

**电池结论(CPU,fp32,两趟:plain top-k 8 核 + KEEP_SAMPLED 2 核)**:十个内核的
`distillation_losses` 与合流前 ch-dev **maxdiff = 0.000e+00**(逐位相等),变动的键
全在诊断面板(`shadow_*` / `tip_teach*`);空响应微批十核全存活(修前 c2 抛);
stat 掩码真被走到(23 行选中 14 行,非回退)且与基线逐位一致;熵分块在 4096/1024/
256/7 四档 maxdiff = 0。**"损失不动、只有诊断动"这条跨 pin 可比性前提成立。**
...
## r6 裁定(2026-08-11):j0_grpo_only 归档 —— J 轴维持有界

用户裁定:j1 的 KD 系数已稀释 500×(β=2e-3),小格实质上是"RL + 一点 KD 正则",
**很难声称仍在研究 OPD** —— 给它补纯 GRPO 控制组等于把战役引向 RL 归因,越出
论文对象。j0_grpo_only 归档不注册;J 轴维持 08-07 的有界裁定(j1 + vanilla_n8
两成员),j1 的归因限制(KD 份额不可知)如实写进其判决 caveat,不再补格。

## r6 登记(2026-08-11):统一损失模板落档(UNIFIED-LOSS.md)+ screening-off 介入格 f2_clip2.3(提案)

**框架落档**(用户裁定:不做方法 arena,用方法推各部件的机理,终点 = findings +
working method)。全名册 43 臂写成单一目标模板的坐标(9 槽:π_gen / w(y) / m_t /
S / T_k / D_β / Φ_M / b / p_T;模板外显式保留 θ₀ 初始化与 J 混合边界),见
`docs/UNIFIED-LOSS.md`。要点:A 轴追认为 λ 剂量线 {0, 0.5, 1}(r6 估计器 caveat
原样携带);对角臂三分 —— 结构性被迫(家族换挡拖动 b/T_k,由 crossover 格 b5/c1
解)、登记混合(E 锚,由 a0 格解)、文献点(受审对象,不作单坐标证据)。指纹
本来就是坐标元组(launcher :229)—— 模板是对既有参数化的显名,不添机器。

**f2_clip2.3(提案,Tier 3)**。压缩剂量线横跨两槽:f 族的界经 Φ_M(显式变换),
b1 的界经 D 内禀(skew α=0.1 ⇒ 估计器上界 ln 10 ≈ 2.303)。量级统一假说因此是
screening-off 命题 —— 塌陷动力学 = g(M),与实现槽无关。本格 = `LOSS_MAX_CLAMP`
10.0→2.3,与 b1 的界配对;零新代码(clamp 走 ARM_ARGS 入指纹 launcher :82/:234,
新 run_id 独立 ckpt 目录,自动分格)。M1 是该假说的**相关性读数**(实测 p99 对
lock 排序),本格是同一假说的**干预试验** —— 名册此前没有任何一格能干预 M 而不换
函数形式。

对立预测:
- **P-M**(量级值充分,screening-off 成立):f2@2.3 的 lock ≈ b1 的 247(种子噪声
  内),动力学与 b1 同族而非 f2@10(198)的平移;M1 实测 p99 两臂重合;
- **P-impl**(实现方式承重):lock 显著偏离 —— 已知机械差异候选:clip 在界外
  **梯度归零**,skew 在界外**平滑饱和**(梯度收缩但非零)。若 P-impl,病原从
  "损失量级"细化为"界外梯度处理",M1 的叙事要改写。

**登记的残余失配**:f2 注册形态 ±10 对称;与 b1 的配对只在正侧精确(k1 重尾所在
侧 —— 学生已采样故 π_θ 不小,教师不认可 ⇒ 大正值),b1 下侧无界。若 P-impl 触发
且归因含糊,不对称精确配格 (−10, 2.3) 是后续格,现在不注册。免费读数:
`clip_hit_rate` 在 2.3 处将远高于 ±10 的命中率 —— 面板已装,直接量"界内还剩多少
信号"。

**成本与 enlistment**:~195 GPU·h(3 seeds,按 f2 外推)+ 30 suite 格;
...
## M1 首次收割(2026-08-11,零新 run):极端尾单调性成立 —— 量级机制的测量半边落地

数据:`docs/data/training_metrics_16k_allkeys.csv.gz`(舰队推送);脚本
`scripts/m1_signal_tail.py`;窗 25–60(25–100 同序)。判决量 = **有效更新信号
极值 u_max**:逐步 `loss_max` 的窗内中位数,逐臂尺度校正(f2 面板在 clamp
上游 → min(·,10);f1 的 loss 即 softlog(r),原始 r 由 e^|u|−1 反解;b1/f3
的 loss 自带界)。

| arm | u_max | lock |
|---|---|---|
| vanilla | 81.6 | 122 |
| f2_hard_clip | 10.0 | 198 |
| f1_soft_log | 4.52 | 208 |
| b1_skew_kl | **2.303** | 247 |
| f3_power | 0.999 | never |

**严格单调,5/5,零反转。** 四条支撑读数:

1. **原始尖峰相同,准入剂量不同**:vanilla/f2/f1 的 raw r_max ≈
   81.6 / 89.2 / 91.1 —— 状态生成的病原暴露基本相同,变换放进更新的剂量不同。
   这同时化解"窗内状态分布已被处理污染"的顾虑:x 轴由 Φ/D 的帽**机械外生**
   决定(10 / log / ln10 / 1 是变换性质),仅 vanilla 的 81.6 依赖状态;
2. **b1 三种子 loss_max 全部 = 2.303 = ln 10(三位小数)**:emergent bound
   从推导变为测量。f2@2.3 的 P-M 预测由此**校准**:u_max 同钉 2.3 ⇒
   lock ≈ 247(种子噪声内);
3. **主体统计全部不排序**:p95 与 lock 反转(f1 1.40 < b1 2.01 而 f1 先锁)、
   absmean 不排、grad_norm 不排(1.30/1.29/0.68/1.00/0.27)—— 只有极端帽
   排序。结合 f2 的 clip_hit_rate = 0.3%:**塌陷剂量住在 ~p99.7 之外的
   token 里**;这与舰队"无梯度统计量领先"一致(全局范数被主体支配),
   p95 的失败本身即是"剂量在极尾"的证据;
4. 截断率窗内已按剂量排序(0.52/0.45/0.37/0.28/0.03)—— 棘轮在窗内
   已按剂量啮合(诚实注:vanilla 无完全 pre-treatment 窗,由 x 轴外生性
   化解)。

**Caveats 与后续**:面板分位网格止于 p95,p99/p99.9 未记 —— waves 9–13 起
建议给 `_QUANTILES` 加 {0.99, 0.999}(metrics-only,zero-fill 先例,不动
...
## r6 记录(2026-08-09):空种群防御 —— 附一条被证伪的诊断

**先更正诊断(2026-08-09 晚,快照证据到手后)**:c4_s1/s2 的实际死因**不是**下面这条
空种群,而是同日早些时候那条**跨设备** bug(`_topk_by_score` 的 `keep & mask`,CPU 掩码
撞 CUDA 张量)。栈帧逐帧指向该行;快照比对给出终局证据——失败的 `20260809_020028`
与其前后几个快照 `grep -c to(teacher_topk_log_probs.device)` = **0**,而 03:31 之后的
快照 = **1**。泳道跑的是**启动时冻结的快照**,所以"s0 跑、s1/s2 死"的不对称与 seed、
与数据毫无关系:**s0 的快照落在修复之后,s1/s2 反复重启时抓到的还是修复前的快照**。

方法论教训记一笔:我从"每次死在同一步"推出"数据依赖、seed 决定",这个推理本身没错,
但**遗漏了第三种确定性来源——冻结的代码快照**。凡是"部分 seed 必死"的现象,先查
`snapshots/<stamp>/src` 的内容,再谈数据。

下面这条空种群修复**依然成立且保留**,它是独立的正确性问题(不是 c4 本次的病因):

根因:`stat` 掩码(响应尾种群)在某些微批里**选不出任何 token**——rollout 立刻吐
EOS、或动态微批恰好只装了这类序列。而三个批级统计量对空张量零容忍:
`torch.quantile` 空输入直接抛(`_topk_by_score`、`_robust_norm`),`min()/max()` 空输入
亦抛(`_minmax`)。CPU 上三处全部复现。c4 是唯一把 stat 传进影子面板的臂,故它最先暴露这条路径;
**d1/d2/d3 自身内核走同一路径**,远程池认领后同样会踩(与那条跨设备 bug 同型:代码
写在"正常批"的假设里)。此修复未在 GPU 上被现场证据触发过,属预防性加固,如实登记。

处置:新增 `_pop(x, mask, who)`——空选择时回退到"无 stat 掩码时本就使用的全体打包种群"
(该回退是本文件既有的、`_stat_mask` 自己也用的口径),每站点一次 stderr 记录以便账本
统计频次。语义上的**非对称是有意的**:阈值种群回退(分位数需要非空输入),可选集仍是
掩码 ⇒ 空批不选任何 token——正确答案,因为那些位置是 prompt/哑行,其损失下游本就丢弃。
[数] 空种群三处不再抛 + `_minmax`/`_robust_norm` 与全体种群逐位一致 + 非空掩码路径逐位
不变;契约测试入 fixture。

## r6 记录(2026-08-09):影子面板的种群不一致 —— 五个内核漏传 stat 掩码

追 c4 的空种群时顺带查出:`_shadow_panel` 跑的是 D 轴三个选择器,它们全是**批级相对**
的;08-07 F3/F4 给 D 轴内核与 c4 补的 stat 掩码(响应尾种群),**c1、c2、e1、e2、e3
这五个只消费面板的内核一直没传**。后果不是崩溃,而是更坏的一类:它们的影子掩码在
含 prompt 与哑行的全打包种群上算(该审计量过哑行能把 TIP 的散度归一化拖约 30 倍),
而 d1/d2/d3 与 c4 在响应尾上算——**V-wave 的冗余判决是跨臂 Jaccard**,半个名册用的
是另一把尺,判决无从成立。

处置:五处全部补传 `_stat_mask(...)`;`_shadow_panel` 函数头写死这条契约(说明为何
必须传、漏传的后果是什么)。CPU 夹具 data 缺席时 stat=None,回退口径不变,五内核冒烟
全绿;既有九个 fixture 原样通过。
