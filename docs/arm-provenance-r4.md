...
## r6 记录(2026-08-09):c4 的"一个 seed 跑、两个 seed 必死"—— 空种群

现象:c4_s0 一路跑到 42 步,s1/s2 每次重启都死在第 1 步。**每次同一步复现**排除了显存
竞争(那会飘),指向**数据依赖的确定性崩溃**:seed 决定数据顺序,s1/s2 的首步微批里
有 s0 碰不到的东西。
...
亦抛(`_minmax`)。CPU 上三处全部复现。c4 是唯一把 stat 传进影子面板的臂,故只有它爆;
**d1/d2/d3 自身内核走同一路径**,远程池认领后必踩(与 08-09 早些时候那条跨设备 bug
同型:代码写在"正常批"的假设里)。
