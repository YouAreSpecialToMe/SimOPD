...
那 8 张是全实验室共享的池子,我们的账户在高优先级分区上**最多拿 2 张**(= 1 条 lane),见 §5。
...
>
> **09-19 再更正。** 初版 §5 说"8 张 A100 全是我们 verification 项目的作业、截稿前不是我们的",**错**。
> 那几个 `p2*` 作业是跑在该节点上的 **CPU 调度器,一张 GPU 都不申请**(`TRES_PER_NODE=N/A`);
> 我当时只看了它们落在哪个节点,没查它们要了什么资源。被占的卡是别人的作业(`PrivateData=jobs` 看不到)。
...
## 5 A100 能拿到多少(09-19 实测)

`nlplarge-compute-01` 是唯一的 A100 节点(8× A100-SXM4-80G),挂在两个分区上,是**全实验室共享**的:

| 分区 | 优先级 | QoS | 我们能拿 | 被抢占 |
|---|---|---|---|---|
| `nlplarge-sasha-highpri` | 30 | `nlplarge-share`:`MaxTRESPA=cpu=64,gres/gpu=2,mem=500G` | **整个 `rush` 账户 ≤ 2 张** | 不会(最高层) |
| `nlplarge` | 20 | 无 | 不设上限,看空闲 | **会** —— `PreemptType=preempt/partition_prio`,`REQUEUE` |

(`MaxTRESPA` 是**每账户**上限,不是每人;`sacctmgr -p show qos nlplarge-share` 实测。)

09-19 读数:8 张里别人占 5 张,空 3 张。

对这 650 步的含义:

- **稳的只有 2 张 = 1 条 lane。**10 条臂串行:H100 上合计 139 lane·h,A100 估 1.5–2 倍,
  即 **约 210–280 h ≈ 9–12 天墙钟**。
- **想并行得走 `nlplarge`**,但高优先级作业一来就被挤掉重排,退回上一个 25 步存档;
  `c5_union_rkl` 攒一个存档要 7.4 h(H100)/ 约 11–15 h(A100 估),在会被抢占的分区上
  可能反复白跑 —— 它该放在那 2 张稳的卡上。
