...
### 轴 B — 散度
Reverse KL(默认)/ Skew-RKL(b1,顶替原 JSD 席位)/ Forward KL / 熵门控(b3);**JSD-β=0.5 于 08-07 入后补批兑现(b4)+ 自研估计器阶梯 k2(b5)**;**β 阶梯 {0.1, 0.9} 于 08-11 兑现(b4_jsd_b0.1 / b4_jsd_b0.9,wave 9)—— b4 落地时登记的内部消融,同核同支撑同分支,β 为唯一变量**。
...
**早停降级为测量(2026-08-06 预注册修订,用户决定)**:所有臂**跑满 H=250,
不再执行早停**;停止规则照常求值,**触发步记入 `early_stops.tsv` 作为逐臂测量**
("撑到第几步"仍是 F 轴数据),但不再 scancel(记录为 `watch.py` 默认行为且可事后重放 —— 规则触发步由完整日志确定,
无需常驻监控;杀须显式 `--kill`,本轮不用)。理由:曲线共享同一 x 轴,跨臂图逐点可比,统计从"最小公共步"
简化为同点比较;且 vanilla_s0 的尾段(0.63@155 → **0.506@250**)证明 Mode A 段的
下坡本身是数据 —— 执行早停会把它截掉。代价 ≈ 每臂 15–20% 机时,预算可容。
注:f1_soft_log 在旧默认(150)下完成,需 RESUME 续至 250 后方可入图。
