...
> **2026-08-24 协议变更(重训波起,k 32 → 8)**:旧集群报废后重训,算力口径重排 —— 一格
> 约 68M token,其中 k=32 的 aime24/aime25/amc23 占 77%,降 k 是唯一不改测量对象的杠杆
> (降 MAX_TOKENS 会直接改掉截断率这个头号仪表)。**k=8 对齐 FiRe 的 Avg@8**;原来的
> k=32 来自 Demystifying 而采样参数来自 Rethinking,本就是混搭。代价已量化:composite 的
> 采样 SE 0.0037 → 0.0075,单格总噪声 0.0056 → 0.0086,**±0.01 的差逐格只有 1.2σ,
> 必须在曲线层面(6 点均值,2.9σ)读**。**`pass@k` 不可跨 k 比较**:已入库 4700 格是
> k=32,重训波是 k=8,只有 avg / composite 可比;产物层面 `newest()` 按每题采样数过滤,
> 两代 parquet 不会互相冒充。开关:`SIMOPD_SUITE_K`(默认仍 32)。
