...
## R5 附录·三方判定裁决(2026-08-19 执行,append-only)

执行:aime24,n=32,τ=0.7,32k 帽,停止集显式 {151643,151645};产物
`$D/evals_r5probe/`(独立目录,含 stop_token_ids 契约列)。

| 腿 | 预测 | 实测(对照 legacy 同格) | 裁决 |
|---|---|---|---|
| P1 a2@25 | 截断自 .985 至少减半 | **.960**(基本不动) | **不成立** |
| P2 vanilla@250 | 基本不动 | **.994**(对照 .858,不降反噪升) | **成立** |
| P3 a1_s2@50 | 居中 | **.118**(对照 .383,stop 88.2%) | **成立(强)** |

**裁决叙述**:M-drift 的"学了句号但采样器拒收"病理在**教师授权 token 的混合臂
上成立且强烈**(P3:a1@50 三分之二的截断是测量假象);在 a2 上**被推翻**——
配套 SFT 探针给出了替代机制:SFT 数据的监督跨度确以真 token `<|im_end|>`(151645)
收尾且不含 `<|endoftext|>`,但产物模型在答案结束位仅给它 p≈0.001(被 `\n\n`
分节符歧义淹没的弱学习),旧约 OPD 又在 50 步内把双终止符全部湮灭(实测
@50 p(im_end)=1e-11)。故终止符病理分**三类**:
P-suppress(句号被 k1 压掉,vanilla 型,实测终止位 Δℓ≈−25 nats)、
P-drift(学了教师句号被采样器拒收,教师混合臂,双停可全额挽回)、
P-untaught(句号从未被教会,a2/SFT 谱系,双停无从挽回)。
vanilla@250 与 a2 谱系的 legacy 曲线是 P-suppress(+P-untaught)复合体;
双停重评只摘除 P-drift 税。分数层面三腿 acc 均无实质变化(这些步位的
AIME 本领本就 ~1-4%),税在 token 成本与终止语义,不在这几格的正确率。
