...
- **08-07 补全审查裁定:J 轴到此为止(用户,成本理由)**——J 臂必须进 n=8 迷你
  cell(4 卡泳道 ×3 seed,单臂成本 = 常规臂 ×2),j1 单代表 + n8 对照已覆盖
  "奖励作共目标"的一阶问题;扩员 = 修正案域。有意划界,非遗漏
...
### H — 序列视界(族 ~7;08-07 证伪括号补全)
- 【审】全 rollout 监督(基线)
- 【审】h1 首段监督(ESR 2605.27028 loss-mask 形,K=100;"信号集中前段"可证伪,
  成立则预算 ×2–47 杠杆;r5 登记形变:生成不截,防 A 轴混淆)
- 【审】h2 末段监督(**08-07 增设**,自研镜像):位置论的证伪杠杆
- 【审】h3 随机连续窗(**08-07 增设**,自研):位置无关预算对照——拆"前段特殊"
  vs"随便 K 个都行"
- 括号纪律:三臂共享 SIMOPD_FIRST_SEGMENT_K=100(预算配平 by construction)、同连续
  窗、同回放归一——**位置是唯一变量**;判读表入臂注。短响应同塌全监督,covered_frac 各报
- 免费面板:k1 族全臂位置分箱 |Δℓ|([0,100)/[100,500)/[500,2k)/[2k,∞)),vanilla
  的曲线即"前段集中"的直接测量,h 臂判决与实测浓度交叉验证
- 【替】ADWIN 自适应窗(2605.28396);Prefix-Guided 预算分配(2606.21994);
  Prune-OPD 漂移截断(2605.07804);Are Full Rollouts Necessary(2605.31490)——
  **边界注:后者与 ESR 真形同属"生成视界"类,截 rollout 改采样分布,若升审必须仿
  n8 前例开自带对照迷你 cell,不得进主表**
- 【落】结构分段(think/答案边界需数据侧掩码,出损失函数接缝)、课程化 K(调度=
  修正案域)、位置衰减软权(Prefix-fade 落点,h1 软化版,"可取未取"登记)
