...
### 2.2 要重训的:**单种子**,共 ~1900 GPU·小时(32 卡 2.5 天 / 64 卡 1.2 天)

**种子数 = 1**(用户 2026-08-24 定)。代价说清楚一次:治愈结论仍然是 **n=1**,
论文里必须把这条限制和结论写在一起,不能只报效应量。

| 批 | 臂 | 步 | 训练 GPU·h | 为什么非它不可 |
|---|---|---|---|---|
| **T1** 判决基线 | `vanilla_corr` | 250 | 119 | 新集群的软硬件栈变了(vLLM/驱动),所有对比都要有本机基线;旧 s0 曲线当跨集群旁证 |
| **T2** 代表面板 | b1 / b2 / c2 / c4_pi_tail / d2 / f2_hard / g6 / h3(**跨 B·C·D·F·G·H 六轴**) | 200 | 668 | 支撑「修正后大家都一样」——六轴各一条即可,不需要 25 条 |
| **T3** 省 token | c3_intersection / h1_first_segment / g2_fire_likelihood(d2 已在 T2) | 200 | 162 | 唯一还开着的正向假说,此前零评测 |
| **T4** c5 预注册 | c5_union_rkl / c5_union_fkl | 250 | 163 | 两条预注册预测至今未决(fkl 的 onset 在 163 步之后) |
| **T5** 机制线索 | e2_set_coverage / h2_last_segment | 200 | 271 | 带修正仍塌的两条 = 第二条塌缩通路 |

**默认地平线 200 步**(只有 T1/T4 到 250):塌缩在 legacy 里 125 步就发生,治愈在 200 步看得一清二楚。

### 2.2b 评测减重:**每 25 步照测,改的是"怎么测"**

步数网格不动(10 个点全测),减重靠采样数 k。成本结构先摆出来 —— **一格约 68M token**:

| benchmark | 题数 × k | 生成条数 | 中位长度 | token 占比 |
|---|---|---|---|---|
| aime24 | 30 × **32** | 960 | 20,266 | 29% |
| aime25 | 30 × **32** | 960 | 17,699 | 25% |
| amc23 | 40 × **32** | 1,280 | 12,173 | 23% |
| minerva | 272 × 3 | 816 | 7,824 | 9% |
| math500 | 500 × 3 | 1,500 | 6,370 | 14% |

**k=32 的三项占 77%**,minerva/math500 加起来才 23% —— 所以杠杆只有一个:**把 k 从 32 降到 16**。

**为什么这不破口径**:composite 用的是 `avg@k` = 逐题正确率的均值,**k 变了估计量不变,
只是方差变大**。代价用已入库数据校准过:

| | composite 的采样 SE(aime+amc 两组) |
|---|---|
| k=32 | 0.0037 |
| **k=16** | **0.0053** |
| k=8 | 0.0075 |

对照:legacy vanilla 三种子的 composite **种子间 sd 中位 0.0042**。所以 k=16 让总噪声从
~0.0056 升到 ~0.0068(**+20%**),换来 **省 38% token**;而我们要读的效应是治愈 +0.10、
"药效归零"的残差 ±0.01 —— k=16 仍分得开,**k=8 会把 ±0.01 那档读糊,不要**。

```
SIMOPD_SUITE_K=16 python scripts/eval_suite.py run --model ... # 或写进 simopd_env.sh 全局生效
```

**两条必须写进论文的说明**:① `pass@k` 与 k 有关,k=16 的 pass **不能**与已入库的
pass@32 相比,只有 avg / composite 可比;② `newest()` 按每题采样数过滤产物,k=16 与
k=32 的 parquet 不会互相冒充(2026-08-07 F1 的 era 门教训),所以新旧混跑不会静默串味。

**不动的**:`MAX_TOKENS=32768`。它看起来很贵,但健康臂回答只有 ~9k、根本用不到帽;
只有塌缩臂才把 32k 跑满 —— 而**截断率正是我们的头号仪表**,降帽等于把测量对象改掉。

**合计:训练 1383 + 评测 ~750(15 臂 × 10 点 × k=16)≈ 2130 GPU·小时**(32 卡 2.8 天 / 64 卡 1.4 天)。保住了每 25 步的分辨率,代价是比六点网格多 230 GPU·h。
