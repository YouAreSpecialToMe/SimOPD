...
>
> **✅ 2026-09-12 核查完毕**(`scripts/analysis/pack_verify.py`,在集群上对 `/home/zz865/opd_pack/extracted` 跑):
> **29 臂、18,242 文件,与 README 自报逐字相符**;29/29 都带齐 `traj/`+`val_gen/`+`metrics/`;
> **`tch_lp_nan` 在 58 个 summary 文件 / 29 条臂上全为 0** —— 教师列修复的收据,也是 09-09 那个切片约定的验证;
> **`div/tok_step*.parquet` 齐全**(每臂 72–749 个),坐实 `results/20260909/archive_inventory.tsv` 的
> `n_tok_parquet=0` 是 `extract.py` 少了一层 `div/` 的 glob bug,**不是没写**。节奏也对:`ids_*` 每步一个、
> `summary_*`/`step_*` 每 25 步一个。
>
> **与 09-09 对账(包是 09-12 打的,中间三天有进展)**:没有任何一条臂在包里比 09-09 少。
> 总步数 **4,325 → 4,780(+455)**,达标臂 **10/29 → 14/29**,进度 **71.5% → 79.0%**。
> 新达标的四条:`b1_skew_kl_corr`(150→200)、`c4_state`(175→200)、`d2_selectkd_corr`(175→200)、
> `f3_power_corr`(175→200)。剩余 15 条共差 **1,270 步**,按剩余量升序:
> f2 50 · e2 52 · c5_union_fkl 54 · a3 59 · g1 59 · a1 74 · b2 75 · n2 75 · h3 79 · h4 86 ·
> a4 100 · c5_union_rkl 105 · h2 127 · g6 134 · a5 141。
