...
旧口径是"每 25 步全评五基准"(每臂 10 点 × 9 GPU·h)。改成:

| 步 | 评什么 | 为什么 |
|---|---|---|
| **100 / 200 / 250** | **五基准全档** | composite 只在五基准齐全时才算,锚点因此**能与已入库的 4700 格直接比较** |
| 25 / 50 / 150 | 只跑 `amc23,minerva,math500` | 给曲线形状与截断率;AIME(30 题 × 32 采样 × 两套、回答最长)约占一格六成成本 |
| 75 / 125 / 175 / 225 | **不排** | 6 个点足够读晚期斜率 |

`python scripts/eval_refill_exp.py --write --grid light --watch 1200`。队列行格式本就是
`run step [benches] [maxtok]`、worker 的 `complete()` 也按行里给的 benchmark 判完成 ——
**这是配置,不是新机制**。每臂 54 → 38 GPU·h,16 臂合计 **864 → 605 GPU·h(省 30%)**。

**合计:训练 1383 + 评测 ~520(只算真正要排的臂)≈ 1900 GPU·小时。**
